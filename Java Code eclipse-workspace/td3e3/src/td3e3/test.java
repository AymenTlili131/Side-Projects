	...
	public class Test3 {
		  ...
		  public static void main(String[] args) {
		    ...
		    for (int i = 0; i < dep.getTabPersonnes().length; i++) {
		      System.out.print("\t" + dep.getTabPersonnes()[i].getNom() + ",\t"
		     + dep.getTabPersonnes()[i].getPrenom());
		 if (dep.getTabPersonnes()[i] instanceof Etudiant)
		   System.out.println(",\t"  
		+ ((Etudiant)dep.getTabPersonnes()[i]).getMatricule()); 
		 else
		   System.out.println(",\t"
		  + ((Enseignant) dep.getTabPersonnes()[i]).getGrade());
		    }
		  }
		} 
}
